    ...
    <div>
        <div class="panel panel-flat border-top-xlg border-top-green-600">
            <div class="panel-heading">
                <h5 class="panel-title"><span class="text-semibold"><i class="icon-table2"></i> Daftar Produk Masuk</span></h5>
            </div>
            <div class="panel-body">
                <div class="pull-left">
                    <div class="col-sm-4">
                        <div class="form-group has-feedback">
                            <input type="text" class="form-control" placeholder="ID Produk Masuk" v-model="id_produk_masuk" v-on:keyup="getDataTransaksiMasuk">
                            <div class="form-control-feedback">
                                <i class="icon-search4 text-size-base"></i>
                            </div>
                        </div>
                    </div>
                    <date-picker v-model="range_date" @change="getDataTransaksiMasuk" :clearable="false" type="date" value-type="format" format="DD-MM-YYYY" range placeholder="Select date range"></date-picker>
                </div>
                <div class="pull-right">
                    <a v-on:click.prevent="showPanel" href="" class="btn bg-green-600 btn-labeled text-bold"><b><i class="icon-plus-circle2"></i></b> Buat Produk Masuk</a>	
                </div>
            </div>
            ...
            <table class="table table-responsive datatable-basic table-xs table-bordered table-framed">
                <thead class="bg-slate-800 text-semibold">
                    <tr>
                        <th class="text-center" width="200">ID Produk Masuk</th>
...
<script>
    ...
    import createmasuk from './Create';
    import axios from 'axios';
    import Swal from 'sweetalert2'
    import 'sweetalert2/src/sweetalert2.scss'

    export default {
        components: { DatePicker, createmasuk},
        data(){
            return {
        ...
        methods: {
            ...
            getDataTransaksiMasuk(){
                axios.get(window.location.origin + '/transaksi/produk-masuk?ajax=true&startdate='+ this.range_date[0] + "&enddate=" + this.range_date[1] + "&kodetransaksi=" + this.id_produk_masuk)
                .then(response => {
            ...
            delete_transaksi(id){
                swal({
						...
						if (result.value) {
							axios.delete(window.location.origin + '/transaksi/produk-masuk/' + id)
                            .then(response => {
                                this.getDataTransaksiMasuk();
                            })
                            .catch(error => {
