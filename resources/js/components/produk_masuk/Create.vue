        ...
        <div class="col-md-12">
            ...
            <div class="panel panel-success" style="border-bottom: none;">
                <div class="panel-heading" style="border-top-right-radius: 0; border-top-left-radius:0;">
                    <h6 class="panel-title">Transaksi Produk Masuk<a class="heading-elements-toggle"><i class="icon-more"></i></a></h6>
                ...
                <div class="panel-body">
                    <form action="" class="form-horizontal">
                        <div class="form-group">
                            <label class="col-sm-2 control-label">ID Produk Masuk :</label>
                            <div class="col-sm-3">
                                <input type="text" class="form-control" placeholder="ID Produk Masuk" v-model="form.kode_transaksi" readonly>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-2 control-label">Tanggal :</label>
                            <div class="col-sm-3">
                                <date-picker type="date" value-type="format" format="DD-MM-YYYY" placeholder="Pilih Tanggl Produk Masuk" v-model="form.tgl_masuk"></date-picker>
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="alert alert-danger alert-styled-right alert-bordered" v-if="errors.list_produk">
                            <span class="text-semibold">{{ errors.list_produk[0] }}</span></a>
                        </div>
                        <div class="table">
							<table class="table">
								<thead>
									<tr>
										<th class="col-lg-4 text-left">Nama Produk</th>
								...
								<tbody>
                                    <template v-if="!form.list_produk.length">
                                    <tr>
                                        <td class="text-center text-bold" colspan="6">Produk Belum Ada</td>
                                    </tr>
                                    </template>
                                    <template v-else>
							        <tr v-for="(item, i) in form.list_produk" :key="item.id">
						                <td>
                                            <input type="hidden" id="produk_id" name="produk_id" v-model="item.produk_id">
                                            <v-select label="nama_produk" :options="options" @search="onSearch" @input="produk => setSelected(item, produk)" v-model="item.nama_produk" placeholder="Silahkan Pilih Produk">
                                                <template slot="no-options">
                                                    Silahkan Masukan Produk
                                                </template>
                                            </v-select>
                                            <div class="label-block" v-if="errors['list_produk.'+i+'.produk_id']">
                                                <span class="help-block text-danger text-bold">{{errors['list_produk.'+i+'.produk_id'][0]}}</span>
                                            </div>
                                        </td>
						                <td>
                                            <vue-numeric 
                                            ...
                                            <div class="label-block" v-if="errors['list_produk.'+i+'.jumlah']">
                                                <span class="help-block text-danger">{{errors['list_produk.'+i+'.jumlah'][0]}}</span>
                                            </div>
                                        </td>
						                ...
						                <td>
                                            ...
                                            <div class="label-block" v-if="errors['list_produk.'+i+'.harga_beli']">
                                                <span class="help-block text-danger">{{errors['list_produk.'+i+'.harga_beli'][0]}}</span>
                                            </div>
                                        </td>
						                <td>
                                            <vue-numeric 
    ...
    export default {
        ...
        data(){
            return {
                ...
                form: {
                    kode_transaksi: '',
                    tgl_masuk: moment().format('DD-MM-YYYY'),
                    catatan: '',
                    list_produk: []
                }
            }
        ...
        mounted(){
            this.getProdukMasukID()
        },
        methods: {
            getProdukMasukID: function(){
                axios.get(window.location.origin + '/getprodukmasukid')
                .then(response => {
                    this.form.kode_transaksi = response.data
                })
                .catch(error => {
                    alert('gagal mengambil kode masuk')
                })
            },
            ...
            remove_line(id){
                this.form.list_produk.splice(id,1);
            },
            closePanel() {
