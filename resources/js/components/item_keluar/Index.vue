    ...
    <div>
        <div class="panel panel-flat border-top-xlg border-top-green-600">
            <div class="panel-heading">
                <h5 class="panel-title"><span class="text-semibold"><i class="icon-table2"></i> Daftar Item Keluar</span></h5>
            </div>
            <div class="panel-body">
                <div class="pull-left">
                    <div class="col-sm-4">
                        <div class="form-group has-feedback">
                            <input type="text" class="form-control" placeholder="ID Item Keluar" v-model="id_item_keluar" v-on:keyup="getDataItemKeluar">
                            <div class="form-control-feedback">
                                <i class="icon-search4 text-size-base"></i>
                            </div>
                        </div>
                    </div>
                    <date-picker v-model="range_date" @change="getDataItemKeluar" :clearable="false" type="date" value-type="format" format="DD-MM-YYYY" range placeholder="Select date range"></date-picker>
                </div>
                <div class="pull-right">
                    <a v-on:click.prevent="showPanel" href="" class="btn bg-green-600 btn-labeled text-bold"><b><i class="icon-plus-circle2"></i></b> Buat Item Keluar</a>	
                </div>
            </div>
            ...
            <table class="table table-responsive datatable-basic table-xs table-bordered table-framed">
                <thead class="bg-slate-800 text-semibold">
                    <tr>
                        <th class="text-center" width="200">ID Item Keluar</th>
...
<script>
    ...
    import createitemkeluar from './Create';
    import axios from 'axios';
    import Swal from 'sweetalert2'
    import 'sweetalert2/src/sweetalert2.scss'

    export default {
        components: { DatePicker, createitemkeluar},
        data(){
            return {
        ...
        mounted(){ 
            this.getDataItemKeluar();
        },
        methods: {
            showPanel() {
                const panelResult = this.$showPanel({
                    component: "createitemkeluar",
                    openOn: 'right',
                    width: 1100
                });
            ...
            getDataItemKeluar(){
                axios.get(window.location.origin + '/kelola-stock/item-keluar?ajax=true&startdate='+ this.range_date[0] + "&enddate=" + this.range_date[1] + "&kodetransaksi=" + this.id_item_keluar)
                .then(response => {
            ...
            delete_transaksi(id){
                swal({
						...
						if (result.value) {
							axios.delete(window.location.origin + '/kelola-stock/item-keluar/' + id)
                            .then(response => {
                                swal(
                                    'Terhapus!',
