...
<table class="table datatable-basic table-xs table-bordered table-framed">
    ...
    <tbody>
        ...
        <tr>
            ...
            @php
                ...
                $tot_pendapatan += $isi->nama_kategori == 'Pendapatan' ? $isi->akun->sum('debit') - $isi->akun->sum('kredit') : 0;
                $tot_beban += $isi->nama_kategori == 'Beban' ? $isi->akun->sum('debit') - $isi->akun->sum('kredit') : 0;
            @endphp
        </tr>
        @endforeach
    </tbody>
    <tfoot>
        <tr class="bg-slate-800">
            ...
            <td colspan="1" class="text-bold text-right">Rp. {{ number_format(abs($tot_pendapatan) - abs($tot_beban), 0) }}</td>
        </tr>
    </tfoot>
</table>
</div>
