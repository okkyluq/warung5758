		...
		<div class="col-sm-6">
			<div class="panel panel-flat border-top-xlg border-top-green-600" id="panel-buat-barang">
				<div class="panel-heading">
					<h5 class="panel-title"><span class="text-semibold"><i class="icon-file-plus"></i> Edit Data Akun</span></h5>
				</div>

				<div class="panel-body">
					<form action="{{ url(Request::segment(1).'/'.Request::segment(2).'/'.$akun->id) }}" method="POST" id="form-akun">
                        @method('PUT')
                        <div class="row form-group">
                            <div class="col-md-5 {{ $errors->has('kategori_akun_id') ? "has-error" : "" }}">
                                <label class="text-bold">*Kategori Akun :</label>
                                <select name="kategori_akun_id" id="kategori_akun_id" class="form-control has-error" disabled>
                                    <option disabled selected value="">Pilih Akun</option>
                                    @foreach ($kategori as $isi)
                                        <option {{ $akun->kategori_akun_id == $isi->id ? 'selected' : '' }} value="{{ $isi->id }}" data-kode="{{ $isi->no_kategori }}">{{ $isi->no_kategori.'. '.$isi->nama_kategori }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-5 {{ $errors->has('kode_akun') ? "has-error" : "" }}">
                                ...
                                <input type="text" class="form-control" id="kode_akun" name="kode_akun" placeholder="Masukan Kode Akun" value="{{ $akun->kode_akun }}" disabled>
                        ...
                        <div class="row form-group">
                            <div class="col-md-10 {{ $errors->has('nama_akun') ? "has-error" : "" }}">
                                ...
                                <input type="text" class="form-control" id="nama_akun" name="nama_akun" placeholder="Masukan Nama Akun" value="{{ $akun->nama_akun }}">
                        ...
                        <div class="row form-group">
                            <div class="col-md-10 {{ $errors->has('status_header') ? "has-error" : "" }}">
                                ...
                                <input disabled type="checkbox" name="status_header" data-on-color="success" data-size="small" class="switch" data-on-text="Ya" data-off-text="Tidak" {{ $akun->status_header ? 'checked="checked"' : '' }}>
                            </div>
                        </div>
                        <div class="row form-group">
                        ...
                        <div class="row form-group">
                            <div class="col-md-4 {{ $errors->has('status_pembayaran') ? "has-error" : "" }}">
                                ...
                                <input disabled type="checkbox" name="status_pembayaran" data-on-color="success" data-size="small" class="switch" data-on-text="Ya" data-off-text="Tidak" {{ $akun->status_pembayaran ? 'checked="checked"' : '' }}>
                            </div>
                            <div class="col-md-4 {{ $errors->has('default_saldo') ? "has-error" : "" }}">
                                ...
                                <input disabled type="checkbox" name="default_saldo" data-on-color="info" data-size="small" class="switch" data-on-text="Debit" data-off-text="Kredit" {{ $akun->default_saldo ? 'checked="checked"' : '' }}>
                            </div>
                        </div>
...
<script>
    ...
    $(document).ready(function(){
        ...
        }).append(akun.parent_akun ? new Option(akun.parent_akun.nama_akun, akun.parent_akun.id, true, true) : null ).prop("disabled", akun.status_header == '1' ? true : false);
    });
